...
contract ResultManager is AccessControlEnumerable {
    ...
    bytes32 public constant RESULT_MANAGER_ADMIN_ROLE = keccak256("RESULT_MANAGER_ADMIN_ROLE");
    ...
    event SignerUpdated(address sender, address indexed prevSigner, address indexed newSigner);
     ...
     */
    function getResult(bytes32 _name) external view onlyRole(FORWARDER_ROLE) returns (uint256, int8, uint256) {
    ...
    function _checkSignature(bytes32 merkleRoot, bytes memory signature) internal view returns (bool) {
        ...
        if (ECDSA.recover(ECDSA.toEthSignedMessageHash(messageHash), signature) != signerAddress) return false;
     ...
     * @param proof The Merkle proof for the result
     */
    function _checkMerkle(Value memory result, bytes32 merkleRoot, bytes32[] memory proof) internal pure returns (bool) {
